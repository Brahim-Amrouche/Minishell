...
/*   Updated: 2023/05/30 21:24:21 by maboulkh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

t_redirection_types    *get_redir_type(char *redir_type)
{
	t_redirection_types *type;

	type = ft_malloc(sizeof(t_redirection_types), m_info(NULL, 1, NULL,0));
	if (!type)
		exit_minishell(ENOMEM, "couldn't malloc redirection info", TRUE);
...
void    parse_redirections(t_list *redir_node, t_minishell *mini)
{
	...
	redir_info = &mini->exec_root->redir;
    if (!redir_node->next)
        exit_minishell(-1, "give me a proper redirection", TRUE);
	type = get_redir_type(redir_node->content);
    redir_info->redir_type = add_element_to_array(redir_info->redir_type,
								type, sizeof(t_redirection_types));
    redir_info->content = add_element_to_array(redir_info->content,
								&redir_node->next->content, sizeof(char *));
